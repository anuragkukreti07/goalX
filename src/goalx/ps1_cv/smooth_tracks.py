...
#     # Updated to match orchestrator naming
#     parser.add_argument("--tracks", required=True, help="Path to projected_tracks.csv")
#     parser.add_argument("--out", required=True, help="Path for smoothed_tracks.csv")
#     parser.add_argument("--window", type=int, default=7)
#     parser.add_argument("--clamp", type=int, default=50)
    
#     args = parser.parse_args()
#     smooth_trajectories(args.tracks, args.out, args.window, args.clamp)
...
def smooth_tracks(df, window=5, max_gap=10, max_speed=50.0):
    df = df.sort_values(["track_id", "frame_id"]).reset_index(drop=True)
    ...
    # Use the 'max_speed' passed from the CLI
    df.loc[df["raw_speed"] > max_speed, ["pitch_x", "pitch_y"]] = np.nan
...
def run_pipeline_step(input_csv, output_csv, window, clamp_val):
    ...
    # Create the jitter report (Before)
    raw_disp = np.sqrt(df.groupby("track_id")["pitch_x"].diff()**2 + 
                       df.groupby("track_id")["pitch_y"].diff()**2)

    # Run smoothing
    smoothed_df = smooth_tracks(df, window=window, max_speed=clamp_val)

    # Jitter report (After)
    sm_disp = np.sqrt(smoothed_df.groupby("track_id")["smooth_x"].diff()**2 + 
                      smoothed_df.groupby("track_id")["smooth_y"].diff()**2)

    print(f"  📊 Jitter (mean px): {raw_disp.mean():.2f} -> {sm_disp.mean():.2f}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser()
    # These argument names now match exactly what run_goalx.py sends
